...
class Config:
    ...
    # ── Datos ────────────────────────────────────────────────────────────────
    DATA_PATH   = r"C:\Users\lr110574\PycharmProjects\Regimenes de Mercado\data\merval.xlsx"
    SHEET_NAME  = 'exc_ret_merval'
    OUTPUT_DIR  = r'R:\Files\User\PORTFOLI\PORFOLIO\Mercados Equity Global\Estrategías equity\Salidas\BOCPD'
...
def ewma_vol(returns: np.ndarray,
    ...
    Al ignorar los retornos positivos, el sizing no penaliza rallies fuertes:
    la exposición solo se reduce cuando el riesgo de caída es elevado.
    """
...
def plot_backtest(df, res, m, bs, out_dir: str):
    ...
    ax2.fill_between(dates, 0, res['w_exec'], color=c['bull'], alpha=0.18)
    ax2.set_ylim(0, 1.5)
    ax2.set_ylabel('Peso equity', color='#8b949e')
    ax2.tick_params(axis='y', colors='#8b949e')
    ax.set_title(f'NAV Acumulado — FCI vs Buy & Hold  |  NAV final ×{m["nav_x"]:.1f}',
                 color=c['bench'])
    ax.legend(fontsize=8, loc='upper left')
    ax.grid(True, alpha=0.4)
    _add_events(ax, dates, ax.get_ylim()[1] if ax.get_ylim()[1] < 1e9 else None)

    # ── B: Señal BOCPD + peso ───────────────────────────────────────────────
    ax = fig.add_subplot(gs[1, :2], sharex=fig.axes[0])
    pb   = res['w_tgt']   # proxy de prob_bull a través del peso objetivo
    bull = res['w_exec'] > 0.01
    ax.fill_between(dates, 0, 1, where=bull,  color=c['bull'], alpha=0.15)
    ax.fill_between(dates, 0, 1, where=~bull, color=c['bear'], alpha=0.08)
    ax.plot(dates, res['w_tgt'],  color='#d2a8ff', lw=0.7, alpha=0.60,
            label='Peso VS objetivo')
    ax.plot(dates, res['w_exec'], color=c['model'], lw=1.2,
            label='Peso ejecutado')
    ax.axhline(0, color='#8b949e', lw=0.5, ls='--', alpha=0.4)
    ax.set_ylim(-0.05, 1.15)
    ax.set_ylabel('Peso equity', color=c['bench'])
    ax.set_title('Peso Portfolio: Régimen × VS20%  (verde=bull, rojo=bear)',
                 color=c['bench'])
    ...
    # ── C: Drawdown ─────────────────────────────────────────────────────────
    ax = fig.add_subplot(gs[2, :2], sharex=fig.axes[0])
    ...
    ax.axvline(0,        color=c['bear'],   lw=1.0, ls='--', alpha=0.7, label='SR = 0')
    ...
    data   = [
        ['Métrica',           'FCI Model',               'Buy & Hold'],
        ['Sharpe',            f'{m["sr"]:+.3f} {sig}',  f'{bh_sr:+.3f}'],
        ...
        ['Sortino',           f'{m["sor"]:.3f}',         '—'],
        ['Ulcer Index',       f'{m["ulcer"]:.4f}',       '—'],
        ['Hit Rate',          f'{m["hit"]:.3f}',         '—'],
        ['SW / año',          f'{m["sw_yr"]:.1f}',       '—'],
        ['% Equity',          f'{m["avg_w"]:.1%}',       '100%'],
        ['TC total',          f'{m["tc_total"]:.1%}',    '—'],
        ['Cash total',        f'{m["cash_total"]:.1%}',  '—'],
    ...
    monthly = pd.Series(res['strat'], index=dates).resample('ME').sum()
    years   = sorted(monthly.index.year.unique())
    Z       = np.full((len(years), 12), np.nan)
    for i, y in enumerate(years):
        for j in range(1, 13):
            sub = monthly[(monthly.index.year == y) & (monthly.index.month == j)]
            if len(sub):
                Z[i, j - 1] = sub.iloc[0]
    vmax = np.nanpercentile(np.abs(Z[~np.isnan(Z)]), 97) if not np.all(np.isnan(Z)) else 0.10
    im   = ax.imshow(Z, cmap='RdYlGn', aspect='auto', vmin=-vmax, vmax=vmax)
    ax.set_xticks(range(12))
    ax.set_xticklabels(['E','F','M','A','M','J','J','A','S','O','N','D'], fontsize=8)
    ax.set_yticks(range(len(years)))
    ax.set_yticklabels(years, fontsize=7)
    ax.set_title('Retornos Mensuales', color=c['bench'], fontsize=8)
    for i in range(len(years)):
        for j in range(12):
            if not np.isnan(Z[i, j]):
                txt = f'{Z[i,j]:.0%}'
                clr = '#0d1117' if abs(Z[i, j]) > vmax * 0.35 else '#e6edf3'
                ax.text(j, i, txt, ha='center', va='center', fontsize=6, color=clr)
    plt.colorbar(im, ax=ax, shrink=0.75)

    fig.suptitle(
        'MERVAL CCL/BADLAR — MODELO FCI: BOCPD × VS20% × TC=15bps × Cash=5%\n'
        f'Período: {dates[0].date()} → {dates[-1].date()}  |  '
        f'Rebalanceo {Config.REBAL_FREQ}  |  N={len(r)} días',
...
qs.reports.html(strategy_returns, bh_returns, title="Backtesting- BOCPD - Exc Ret Merval",
                output=r'R:\Files\User\PORTFOLI\PORFOLIO\Mercados Equity Global\Estrategías equity\Salidas\BOCPD\BT_BOCPD_Merv_exc_ret.html')
webbrowser.open( r'R:\Files\User\PORTFOLI\PORFOLIO\Mercados Equity Global\Estrategías equity\Salidas\BOCPD\output\BT_Hist_Estrategia MERV.html' )
