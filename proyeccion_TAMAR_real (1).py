...
class Config:
    ...
    output_traj_xlsx: str = "trayectoria_tamar.xlsx"   # trayectorias para el valuador de opciones
    n_export_paths: int = 5000                          # paths a exportar (subconjunto de n_sim)
...
def run_scenarios(r0: float, kvec: np.ndarray, svec: np.ndarray, steps: int,
    ...
    for name, c in cfg.regimes.items():
        ...
        res[name] = dict(
            ...
            all_paths=sim,                  # paths COMPLETOS (para exportar trayectorias)
...
# =====================================================================================
# 6b. EXPORTACIÓN DE TRAYECTORIAS (formato consumido por valuacion_opcion_dual_prod.py)
# =====================================================================================
def export_trajectories(idx: pd.DatetimeIndex, res: Dict[str, dict], cfg: Config) -> None:
    """
    Exporta las trayectorias COMPLETAS de real_TAMAR en el formato EXACTO que lee el
    valuador de opciones (Config de valuacion_opcion_dual_prod con
    months_in_rows=False, has_index_col=True, paths_are_monthly_real=True):

      - 1 hoja por régimen, nombre 'sims_<régimen>'  (prefijo que busca el valuador)
      - filas = paths, columnas = meses (etiquetadas 'm{n}_AAAA-MM'), 1ra col = índice 'path'
      - valores = tasa real MENSUAL en decimal (sin anualizar, sin nominal)

    CONSISTENCIA: usa los MISMOS paths simulados que el modelo (res['all_paths']),
    de modo que el valuador trabaja sobre exactamente las trayectorias analizadas acá.
    Se exporta un subconjunto de n_export_paths (≤ n_sim) para que el archivo sea
    liviano y abrible; al ser los primeros paths, sigue siendo el mismo set del modelo.
    """
    def _safe_sheet(name: str) -> str:
        for ch in r'[]:*?/\\':
            name = name.replace(ch, " ")
        return f"sims_{name}".strip()[:31]              # límite de 31 caracteres de Excel

    n = min(cfg.n_export_paths, cfg.n_sim)
    mlabels = [f"m{j+1}_{d.strftime('%Y-%m')}" for j, d in enumerate(idx)]
    with pd.ExcelWriter(cfg.output_traj_xlsx) as xl:
        for name, r in res.items():
            arr = r["all_paths"][:n]                    # (n, steps) real mensual
            df = pd.DataFrame(arr, columns=mlabels)
            df.index.name = "path"
            df.to_excel(xl, sheet_name=_safe_sheet(name))
    print(f"Trayectorias guardadas: {cfg.output_traj_xlsx}  "
          f"({n} paths × {len(idx)} meses × {len(res)} regímenes)")
    print("  -> alimentar valuacion_opcion_dual_prod.py con traj_path apuntando a este archivo.")
    print("  -> si usás regime_weights en el valuador, las claves son los nombres de hoja "
          "('sims_<régimen>').")
...
def main(cfg: Config = Config()) -> None:
    ...
    export_trajectories(idx, res, cfg)
