...
class Config:
    ...
    months_in_rows: bool = False        # paths en FILAS, meses en columnas               # orientación: meses en filas, paths en columnas
    ...
    bond_life_m: int = 25
    cer_real_spread: float = 0.04             # pata CER del dual: CER + spread  -> STRIKE
    tamar_margin: float = 0.03                # pata TAMAR del dual: TAMAR + margen
    lead_ratio: float = 0.9846                # VT_pata_TAMAR / VT_pata_CER a HOY (seasoning).
                                              # 1.0 = bono nuevo (patas parejas); >1 = TAMAR adelantada.
                                              # 0.9846 = (1+3.18%)/(1+4.79%): CER adelante hoy.

    # --- TIR / descuento ---
    real_discount_rate: float = 0.076         # TIR real para traer el payoff a hoy
...
def value_option(real_paths: np.ndarray, cfg: Config) -> dict:
    ...
    # lead_ratio escala la pata TAMAR por su devengamiento YA acumulado vs la CER (seasoning).
    # Numerario = valor técnico de la pata CER hoy (=1); la pata TAMAR vale lead_ratio hoy.
    leg_tamar = cfg.lead_ratio * np.prod(1.0 + paths + cfg.tamar_margin / 12.0, axis=1)   # estocástica
